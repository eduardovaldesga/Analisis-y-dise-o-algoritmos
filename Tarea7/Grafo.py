...
class Nodo:
    ...
    def __init__(self,info_nodo):
        ...
        [self.id,self.x,self.y,self.on,self.demanda]=[str2number(m) for m in info_nodo]
    def __repr__(self):
        return "%s\t%s\t%s\t%s\t%s\t" %(self.id,self.x,self.y,self.on, self.demanda)
    def __hash__(self):
        return hash(self.id)
...
class Arco:
    ...
    def __init__(self,info_arco):
        ...
        self.start,self.end,self.on,self.longitud,self.topologia=[str2number(m) for m in info_arco]
    def __repr__(self):
        return "%s\t%s\t%s\t%s\t%s\t" %(self.start,self.end,self.on,self.longitud,self.topologia)
...
#                    assert hasta in self.lista.keys()
            ...
            input.close()
            self.N=self.numNodos+self.numSubE
